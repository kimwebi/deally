...
- The package's central console uses a warm, hand-rolled CSS theme (`resources/css/app.css`, published to `public/vendor/multitenant/saas-foundation/css`): flash stat cards in blue/black/yellow/gold, a pure-CSS donut chart for the customer-status mix, a bar graph by status, and a **customer milestone** — the dashboard and setup console show a gold progress bar toward `CentralDashboardController::MILESTONE_TARGET` (10 customers), and switch to a celebratory "Hurray! 🎉" banner once the target is reached.
